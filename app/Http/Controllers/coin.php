	...
	private function generateData()
	{
		...
		foreach ($coinList as $one) {
			if (Storage::exists('' . strtolower($one['coin']) . '.json')) {
				...
				$setData['totalMasterNodes'] = $coinData['masterNodeCount']['enabled'];
				$setData['masterNodeCoinsRequired'] = $coinData['coinData']['masterNodeCoinRequired'];
				$setData['income'] = $coinData['last24Hours']['perNode']['values']['price_usd'];
				$setData['lastUpdated'] = $coinData['lastUpdated'];
				$setData['cmc'] = $coinData['cmcData'];
				$setData['market_cap'] = $coinData['cmcData']['market_cap_usd'];
				$setData['coin_supply'] = $coinData['cmcData']['available_supply'];
				$setData['percent_change_24h'] = $coinData['cmcData']['percent_change_24h'];
				$setData['coinLocked'] = $coinData['coinLocked']['total'];
				$setData['coinLockedPercent'] = $this->coinLockedPercent($coinData['coinLocked']['total'], $coinData['cmcData']['available_supply']);
				$setData['dailyRev'] = number_format($coinData['last24Hours']['perNode']['values']['price_usd']['daily'], '2', '.', '');
				$setData['weeklyRev'] = number_format($coinData['last24Hours']['perNode']['values']['price_usd']['weekly'], '2', '.', '');
				$setData['monthlyRev'] = number_format($coinData['last24Hours']['perNode']['values']['price_usd']['monthly'], '2', '.', '');
				$setData['yearlyRev'] = number_format($coinData['last24Hours']['perNode']['values']['price_usd']['yearly'], '2', '.', '');
				$setData['coin'] = $one['coin'];
				$setData['name'] = $one['name'];
				$setData['roi'] = number_format($coinData['last24Hours']['perNode']['values']['price_usd']['roi'], '2', '.', '');
				$setData['realRoi'] = $setData['roi'];
				$setData['logo'] = $coinData['coinData']['logo'];
				if (isset($one['ads'])) {
					$setData['ads'] = $one['ads'];
					$setData['url'] = $one['url'];
				}
				if (isset($one['disabled'])) {
					$setData['roi'] = 'offline';
					$setData['dailyRev'] = 'offline';
					$setData['weeklyRev'] = 'offline';
					$setData['monthlyRev'] = 'offline';
					$setData['yearlyRev'] = 'offline';
					$setData['realRoi'] = 0;
					$setData['disabled'] = $one['disabled'];
				}
				if (isset($one['off'])) {
				} else {
					$data['coinList'][$one['coin']] = $setData;
				}
			}
		}
		return $data;
